...
export default function ModuleNav() {
  ...
  return (
    <nav
      ...
      className="w-[280px] bg-white border-r border-gray-200 h-screen flex flex-col flex-shrink-0"
    >
      <div className="p-5 border-b border-gray-100">
        <button
          className="flex items-center gap-2 text-xs font-bold text-gray-500 hover:text-primary-500 mb-4 transition focus:outline-none focus:ring-2 focus:ring-primary-500 rounded"
          onClick={() => router.push('/dashboard/student')}
        >
          <ArrowLeft size={14} /> All courses
        </button>
        <div className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{course.code}</div>
        <div className="text-lg font-bold text-gray-900 mt-1">{course.title}</div>
            ...
            {lesson.modules.map((m) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 px-5 py-2.5 cursor-pointer transition-all border-l-4 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-primary-500 ${
                    isActive
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div
                      className={`text-sm font-medium truncate ${isActive ? 'text-primary-700 font-bold' : ''} ${isDone ? 'text-gray-400 line-through' : ''} ${!isDone && !isActive ? 'text-gray-800' : ''}`}
                    >
                      {m.title}
                    </div>
